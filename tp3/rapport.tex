...
\\
Pour notre deuxième essai, nous avons décidé de structurer nos tests comme plusieurs \epmh{working-set} se succédant dans le temps, ce qui est nous 
...
temps à être remplacée lors de la transition d’un \epmh{working-set} à un autre (et une page commune aux deux \epmh{working-set} ne sera pas remplacée).
\\
Par soucis de simplicité, les \epmh{working-set} ont été défini comme un ensemble de pages contiguës en mémoire, permettant la génération aléatoire 
...
deux \epmh{working-set} de 16 pages chacun (pages 0-15 et 12-27), avec un chevauchement de 4 pages communes. Chaque \epmh{working-set} est composé de 30 requêtes. 
\\
...
donc la quasi-totalité de l’adressage logique. Pour éviter que le test ne soit trop long, nous avons défini des \epmh{working-set} avec un nombre de 
pages plus élevé, bien que ce changement engendre évidement une baisse de l’efficacité du TLB. Le test est composé de 3 \epmh{working-set} de 20 pages
